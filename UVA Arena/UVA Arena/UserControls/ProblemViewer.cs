...
namespace UVA_Arena.Elements
{
    public partial class ProblemViewer : UserControl
    {
        ...
        private void ShowDiscuss()
        {
            if (current == null)
            {
                customWebBrowser1.Tag = null;
                customWebBrowser1.Navigate("http://acm.uva.es/board/");
                return;
            }

            if (customWebBrowser1.Tag != null && customWebBrowser1.Tag.Equals(current.pnum)) return;
            string query = string.Format("search.php?keywords={0}", current.pnum);
            string discuss = string.Format("http://acm.uva.es/board/{0}", query);
            customWebBrowser1.Navigate(discuss);
            customWebBrowser1.Tag = current.pnum;
        }
