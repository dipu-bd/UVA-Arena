...
void TestSampleData(uhuntqt::Uhuntqt& api)
{
    cout << "Sample data: " << endl;
    for (uhuntqt::ProblemInfo info : api.problemListFromData(sampleData))
    {
...
int main(int argc, char* argv[])
{
    ...
    uhuntqt::Uhuntqt api(manager);

    QObject::connect(&api, &uhuntqt::Uhuntqt::problemListDownloaded, 
        [] (QList<uhuntqt::ProblemInfo> data)
        {
            for (uhuntqt::ProblemInfo info : data)
            {
                cout << info.getProblemTitle().toStdString() << endl;
            }
    ...
    QObject::connect(&getProblemListButton, &QPushButton::clicked,
                     &api, &uhuntqt::Uhuntqt::getProblemList);
